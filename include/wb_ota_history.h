...
struct Entry {
    ...
    String   version;        // for "ota": running firmware at upload time (the FROM version)
                             // for "boot": firmware that successfully reached healthy
    String   target;         // for "ota": version the upload installed (the TO version), when known
...
// Record an OTA upload attempt. `was_running` is the version that performed
// the upload — the OLD (FROM) firmware. The TO version is normally NOT known
// here (the incoming image's app-descriptor version is the IDF version, not
// our WB_VERSION), so `target` defaults to "" and is backfilled by
// recordBoot() when the new firmware boots. Pair with recordBoot() to get the
// "did the new version actually run" half.
