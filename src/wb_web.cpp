...
function saveEco(){
  ...
  /* Documented Wallbox eco_smart enum (#38): esm 0 = Eco (grid+solar),
     1 = Full Green. There is no esm 2 — the charger ignores it. The dropdown
     carries semantic tokens ('off'/'green'/'eco') so "Solar + Grid" maps to
     esm 0, not the old invalid 2. */
  var mode=document.getElementById('eco-mode').value;
  ...
  if(mode==='off'){
    /* Disabling: the charger ignores an esm change sent together with ese=0,
       ...
       assert with the flag still ON, then drop the flag (matches the MQTT
       path / issue #29). */
    sEco({ese:1,esm:0,esp:esp}).then(function(){return sEco({ese:0,esm:0,esp:esp})}).then(done).catch(err);
  }else{
    var esm=(mode==='green')?1:0;   /* 'eco' (Solar + Grid) => esm 0 */
    sEco({ese:1,esm:esm,esp:esp}).then(done).catch(err);
  }
}
